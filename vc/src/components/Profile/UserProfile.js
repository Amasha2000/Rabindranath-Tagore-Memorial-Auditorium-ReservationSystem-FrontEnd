...
const UserProfile = () => {
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            const username = localStorage.getItem('vcName');
            try {
                const response = await axios.get(`http://localhost:8080/vc/${username}`);
    ...
    const handleFormSubmit = async (e) => {
        ...
        const username = localStorage.getItem('vcName');
        try {
            await axios.put(`http://localhost:8080/vc/${username}`, formData);
